...
class ConvBase(nn.Module):
    def __init__(self, observation_space, output_size=256):
        ...
        self.conv2 = nn.Conv2d(self.obs_channel * 2, self.obs_channel * 2, 3, stride=1, padding=1)
        self.conv3 = nn.Conv2d(self.obs_channel * 2, self.obs_channel * 3, 3, stride=2, padding=1)
        self.conv4 = nn.Conv2d(self.obs_channel * 3, self.obs_channel * 3, 3, stride=2, padding=1)

        self.bn1 = nn.BatchNorm2d(self.obs_channel * 2)
        self.bn2 = nn.BatchNorm2d(self.obs_channel * 2)
        self.bn3 = nn.BatchNorm2d(self.obs_channel * 3)

        obs_channel = self.obs_channel * 3
        obs_width = int(((self.obs_width + 1) // 2 + 1) // 2)
        obs_height = int(((self.obs_height + 1) // 2 + 1) // 2)
        self.fc1 = nn.Linear(obs_channel * obs_height * obs_width, 512)
        self.fc_bn1 = nn.LayerNorm(512)
