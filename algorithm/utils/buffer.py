...
from algorithm.utils import get_shape_from_space
...
class ReplayBuffer(Buffer):
    ...
    def __init__(self, args, num_agents, obs_space, act_space):
        ...
        self.obs = np.zeros((self.buffer_size + 1, self.n_rollout_threads, self.num_agents, *obs_shape),
                            dtype=np.float32)
        # print('[buffersize,n_rollout_threads,num_agents,Xx_shape]')
        ...
        # pi(a)
        self.action_log_probs = np.zeros((self.buffer_size, self.n_rollout_threads, self.num_agents, 1),
                                         dtype=np.float32)
    ...
    def recurrent_generator(buffer: Union[Buffer, List[Buffer]], num_mini_batch: int, data_chunk_length: int):
        ...
        for indices in sampler:
            ...
            rnn_states_critic_batch = np.stack(rnn_states_critic_batch).reshape(N,
                                                                                *buffer[0].rnn_states_critic.shape[3:])
    ...
    @staticmethod
    def generator(buffer: Union[Buffer, List[Buffer]], num_mini_batch):
        """
        A generator that yields training data for MLP training arranged in mini batches.
        This generator shuffles the data by sequences.
        ...
        Returns:
            (obs_batch, actions_batch, masks_batch, old_action_log_probs_batch, advantages_batch, \
                returns_batch, value_preds_batch, None, None)
        """
        buffer = [buffer] if isinstance(buffer, ReplayBuffer) else buffer  # type: List[ReplayBuffer]
        n_rollout_threads = buffer[0].n_rollout_threads
        buffer_size = buffer[0].buffer_size
        ...
        # Get mini-batch size and shuffle chunk data
        mini_batch_size = n_rollout_threads * buffer_size // num_mini_batch
        rand = torch.randperm(n_rollout_threads * buffer_size).numpy()
        sampler = [rand[i * mini_batch_size:(i + 1) * mini_batch_size] for i in range(num_mini_batch)]
        ...
        for indices in sampler:
            ...
            for index in indices:
                # size [T+1, N, Dim] => [T, N, Dim] => [N, T, Dim] => [N * T, Dim] => [L, Dim]
                obs_batch.append(obs[index:index+1])
                actions_batch.append(actions[index:index+1])
                masks_batch.append(masks[index:index+1])
                old_action_log_probs_batch.append(old_action_log_probs[index:index+1])
                advantages_batch.append(advantages[index:index+1])
                returns_batch.append(returns[index:index+1])
                value_preds_batch.append(value_preds[index:index+1])
            ...
            # Flatten the (L, N, ...) from_numpys to (L * N, ...)
            obs_batch = ReplayBuffer._flatten(1, N, obs_batch)
            actions_batch = ReplayBuffer._flatten(1, N, actions_batch)
            masks_batch = ReplayBuffer._flatten(1, N, masks_batch)
            old_action_log_probs_batch = ReplayBuffer._flatten(1, N, old_action_log_probs_batch)
            advantages_batch = ReplayBuffer._flatten(1, N, advantages_batch)
            returns_batch = ReplayBuffer._flatten(1, N, returns_batch)
            value_preds_batch = ReplayBuffer._flatten(1, N, value_preds_batch)
...
class SharedReplayBuffer(ReplayBuffer):
    ...
    def insert(self,
        ...
        if available_actions is not None:
            pass
        return super().insert(obs, actions, rewards, masks, action_log_probs, value_preds, rnn_states_actor,
                              rnn_states_critic)
