...
class Simple115StateWrapper(gym.ObservationWrapper):
  ...
  def __init__(self, env, fixed_positions=False):
    ...
    shape = (action_shape[0] if len(action_shape) else 1, 32)
  ...
  def convert_observation(observation, fixed_positions):
    ...
    for obs in observation:
      ...
      if fixed_positions:
        ...
        o.extend(obs['ball'][:2]) # length:14
        o.extend(do_flatten(obs['left_team']))   #length:18
        o.extend(do_flatten(obs['right_team']))  #length:22
        o.extend(obs['ball_direction'][:2])     #length:24
        o.extend(do_flatten(obs['left_team_direction']))  #length:28
        o.extend(do_flatten(obs['right_team_direction'])) #length:32
        #length:32
